...
public class BookModel {
    ...
    public func importBook(from url: URL) async {
        ...
        let book = await importBookWithFallback(from: url)
        index = Index(book: book, audioFileDirectory: .bookDirectory)
        print("Importing took \(Date.now.timeIntervalSince(start)) seconds.")
    }

    /// Searches the book for the given `String`.
    public func searchResults(for searchString: String) async -> [SearchResultSection] {
        return await index?.searchResults(for: searchString) ?? []
    }
    ...
    ///
    @concurrent
    private func importBookWithFallback(from url: URL) async -> Book? {
        ...
        do {
            ...
            if url == .defaultBook {
                ...
                // Failed to load a book, revert to the default book.
                return await importBookWithFallback(from: .defaultBook)
            }
        }
    }
    ...
    ///
    nonisolated private func loadBook() throws -> Book {
